...
async function main() {
  ...
  // Clear existing data
  await prisma.noShowHistory.deleteMany({});
  ...
  const maria = await prisma.patient.create({
    data: {
      firstName: 'Maria',
      lastName: 'Rodriguez',
      phoneNumber: '+13108678464', // Demo phone number for real SMS testing
  ...
  // Patient 2: James Wilson - High risk, NO phone, appointment tomorrow 2 PM
  ...
  // Patient 3: Carlos Martinez - Medium risk, has phone
  ...
  // Patient 4: Lisa Chen - High risk, has phone, many no-shows
  ...
  // Patient 5: David Kim - Low risk (housed), for comparison
  ...
  const mariaAppointment = await prisma.appointment.create({
    data: {
      ...
      riskScore: 85, // Pre-calculated: Homeless(40) + Distance(20) + 2 NoShows(30) - 5 = 85
      needsRide: true,
      rideOfferSent: false
    }
  });
  ...
  const jamesAppointment = await prisma.appointment.create({
    data: {
      ...
      status: AppointmentStatus.SCHEDULED,
      riskScore: 95, // Homeless(40) + Distance(30) + NoPhone(25) = 95
      needsRide: true,
      rideOfferSent: false
    }
  });
  ...
  const carlosAppointment = await prisma.appointment.create({
    data: {
      ...
      status: AppointmentStatus.SCHEDULED,
      riskScore: 50, // Unstably Housed(25) + Distance(10) + 1 NoShow(15) = 50
      needsRide: false,
      rideOfferSent: false
    }
  });
  ...
  const lisaAppointment = await prisma.appointment.create({
    data: {
      ...
      status: AppointmentStatus.SCHEDULED,
      riskScore: 100, // Homeless(40) + Distance(30) + 3 NoShows(30, capped at 60) = 100 -> capped at 100
      needsRide: true,
      rideOfferSent: false
    }
  });

  const davidAppointment = await prisma.appointment.create({
    data: {
      patientId: david.id,
      clinicId: clinic.id,
      ...
      status: AppointmentStatus.SCHEDULED,
      riskScore: 10, // Housed(0) + Distance(<2 miles = 10) = 10
      needsRide: false,
      rideOfferSent: false
    }
  });
  ...
  // Create no-show history for realistic risk scoring
  ...
  // Create sample SMS conversation for Maria (showing a previous successful interaction)
    ...
    data: [
      {
        ...
        message: 'Hi Maria! You have an appointment on ' + tomorrow.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) + ' at 10:00 AM at Mission Health Clinic. Need a free ride? Reply YES and we\'ll send an Uber to pick you up.',
  ...
  // Carlos already declined a ride offer
  await prisma.sMSMessage.createMany({
    data: [
      {
        appointmentId: carlosAppointment.id,
        phoneNumber: '+15552345678',
        message: 'Hi Carlos! You have an appointment on ' + dayAfterTomorrow.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) + ' at 9:00 AM at Mission Health Clinic. Need a free ride? Reply YES and we\'ll send an Uber to pick you up.',
        direction: SMSDirection.OUTBOUND,
        status: SMSStatus.DELIVERED,
      ...
      },
      {
        appointmentId: carlosAppointment.id,
        phoneNumber: '+15552345678',
        message: 'No problem! Let us know if you need anything else. See you at your appointment!',
        direction: SMSDirection.OUTBOUND,
        status: SMSStatus.DELIVERED,
  ...
  // Update Carlos's appointment to reflect he doesn't need a ride
  ...
  console.log('Summary:');
  console.log('- 1 Clinic: Mission Health Clinic');
  console.log('- 1 Caseworker: Sarah Johnson');
  console.log('- 5 Patients with varying risk levels');
  console.log('- 5 Upcoming appointments');
  console.log('- Sample no-show history');
  console.log('- Sample SMS conversations\n');

  console.log('Patient Risk Scores:');
  console.log('- Maria Rodriguez: 85 (HIGH) - Tomorrow 10 AM');
  console.log('- James Wilson: 95 (HIGH) - Tomorrow 2 PM - NO PHONE, uses caseworker');
  console.log('- Carlos Martinez: 50 (MEDIUM) - Day after tomorrow - declined ride');
  console.log('- Lisa Chen: 100 (HIGH) - 3 days out');
  console.log('- David Kim: 10 (LOW) - 4 days out');
}
